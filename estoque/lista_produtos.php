...
const table = new Tabulator("#lista", {
	...
	columns:[
			...
			{title: "Açoes", headerSort:false, width: 155, formatter: cell =>	'<div class="flex items-center justify-center gap-1.5">' + `<a href="editar.php?id=${cell.getRow().getData().id_estoque}" class="mr-3 bg-yellow-600 hover:bg-yellow-700 text-gray-200 inline-flex items-center text-center px-2 rounded-md" role="button">Editar</a>` +

			`<button onclick="onSubmit(${cell.getRow().getData().id_estoque})" class="bg-red-700 hover:bg-red-800 text-gray-200 inline-flex items-center text-center px-2 rounded-md" type="button">Excluir</button>` +
			'</div>'
			}
		],
		locale: "pt-BR",
