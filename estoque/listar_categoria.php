...
<script>
	async function onSubmit(id) {
		...
		const response = await fetch(`delete_categorie.php?id=${id}`,{
			method:"delete",
		})
		if(response.ok){
...
const table = new Tabulator("#lista", {
	...
	columns:[
		...
		'<div class="flex items-center justify-center gap-1.5">' +
			...
			`<button onclick="onSubmit(${cell.getRow().getData().id_categoria})" class="bg-red-700 hover:bg-red-800 text-gray-200 inline-flex items-center text-center px-2 rounded-md" role="button">Excluir</button>` +
		'</div>'
			}
		],
		locale: "pt-BR",
